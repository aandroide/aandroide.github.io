...
def set_key():
    ...
    if new_key and saved_key != new_key:
        ...
        data = '<keymap><global><keyboard><key id="%s">' % new_key + 'runplugin(plugin://plugin.video.lo-scienziato-pazzo/?ew0KICAgICJhY3Rpb24iOiAia2V5bWFwIiwNCiAgICAib3BlbiI6IHRydWUNCn0=)</key></keyboard></global></keymap>'
    ...
    def onClick(self, control_id):
        if control_id in [1, 2]:
            ...
            if Item().fromurl(action).folder == False:
                xbmc.executebuiltin('RunPlugin("plugin://plugin.video.lo-scienziato-pazzo/?' + action + '")')
            else:
                xbmc.executebuiltin('ActivateWindow(10025, "plugin://plugin.video.lo-scienziato-pazzo/?' + action + '")')
