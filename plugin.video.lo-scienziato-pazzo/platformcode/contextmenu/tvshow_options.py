    ...
    if check_condition():
        ...
        if item:
            ...
            items.append((update_text,
                          lambda: xbmc.executebuiltin("RunPlugin(plugin://plugin.video.lo-scienziato-pazzo/?{}&title={}&action=mark_tvshow_as_updatable&channel=videolibrary&active={})".format(item_url, update_text, str(value)))))
            if item.local_episodes_path == "":
                items.append((config.get_localized_string(80048), lambda: xbmc.executebuiltin("RunPlugin(plugin://plugin.video.lo-scienziato-pazzo/?{}&action=add_local_episodes&channel=videolibrary&path={})".format(item_url, path))))
            else:
                items.append((config.get_localized_string(80049), lambda: xbmc.executebuiltin("RunPlugin(plugin://plugin.video.lo-scienziato-pazzo/?{}&action=remove_local_episodes&channel=videolibrary&path={})".format(item_url, path))))
            ...
            if channels_num > 1:
                ...
                multichannel = False
            items.append((delete_text, lambda: xbmc.executebuiltin("RunPlugin(plugin://plugin.video.lo-scienziato-pazzo/?{}&action=delete&channel=videolibrary&multichannel={}&path={})".format(item_url, str(multichannel), path))))
        ...
        #         Download movie
                # if mediatype == "movie":
                #     items.append((config.get_localized_string(60354), lambda: xbmc.executebuiltin("RunPlugin(plugin://plugin.video.lo-scienziato-pazzo/?%s&%s)" % (item_url,
                ...
                #     Download series
                    ...
                    #         items.append((config.get_localized_string(60003), lambda: xbmc.executebuiltin("RunPlugin(plugin://plugin.video.lo-scienziato-pazzo/?%s&%s)" % (
                    #             item_url,
                    #             'channel=downloads&action=save_download&unseen=true&from_channel=' + item.channel + '&from_action=' + item.action))))
                    #     items.append((config.get_localized_string(60355), lambda: xbmc.executebuiltin("RunPlugin(plugin://plugin.video.lo-scienziato-pazzo/?%s&%s)" % (
                    #         item_url,
                    #         'channel=downloads&action=save_download&from_channel=' + item.channel + '&from_action=' + item.action))))
                    #     items.append((config.get_localized_string(60357), lambda: xbmc.executebuiltin("RunPlugin(plugin://plugin.video.lo-scienziato-pazzo/?%s&%s)" % (
                    #         item_url,
                    ...
                    #     items.append((config.get_localized_string(60356), lambda: xbmc.executebuiltin("RunPlugin(plugin://plugin.video.lo-scienziato-pazzo/?%s&%s)" % (
                    #         item_url,
                    #         'channel=downloads&action=save_download&from_channel=' + item.channel + '&from_action=' + item.action))))
                    # Download season
