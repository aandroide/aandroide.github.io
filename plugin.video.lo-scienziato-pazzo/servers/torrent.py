...
def elementum_download(item):
    ...
    if elementum_setting:
        ...
        if config.get_setting('downloadpath').startswith('smb'):
            ...
            if select:
                xbmc.executebuiltin("RunPlugin(plugin://plugin.video.lo-scienziato-pazzo/?eyJjaGFubmVsIjoic2hvcnRjdXRzIiwgImFjdGlvbiI6IlNldHRpbmdPblBvc2l0aW9uIiwgImNhdGVnb3J5Ijo2LCAic2V0dGluZyI6MX0=)")
        else:
